...
from app.schemas.pamphlet import PamphletCreate, Pamphlet, PamphletVersionCreate, PamphletVersion
from app.models.pamphlet import Pamphlet as PamphletModel, PamphletVersion as PamphletVersionModel
...
def create_pamphlet_version(
    ...
    try:
        minio_client.upload_fileobj(
            file.file,
            settings.MINIO_BUCKET_PAMPHLETS,
            file_path
        )
...
def list_pamphlets(
    ...
    if search:
        ...
        query = query.filter(
            search_vector.op("@@")(func.plainto_tsquery("simple", search))
        )
