...
from app.models.pamphlet import PamphletFork, MergeRequest, MergeRequestStatus
from app.schemas.pamphlet import (
    PamphletForkCreate, PamphletFork, 
    MergeRequestCreate, MergeRequestUpdate, MergeRequest
)
...
def create_fork(
    ...
    """
    from app.models.pamphlet import Pamphlet, PamphletVersion
    from app.schemas.pamphlet import PamphletCreate
    
    # Get original pamphlet
    original = db.query(Pamphlet).filter(
        Pamphlet.id == pamphlet_id,
        Pamphlet.is_public == True
    ).first()
    if not original:
        raise HTTPException(
    ...
    # Get latest version
    latest_version = db.query(PamphletVersion).filter(
        PamphletVersion.pamphlet_id == pamphlet_id
    ).order_by(PamphletVersion.version_number.desc()).first()
    ...
    new_pamphlet = Pamphlet(
        **new_pamphlet_data.dict(),
    ...
    # Copy version
    new_version = create_pamphlet_version_from_existing(
        db=db,
...
def create_merge_request(
    ...
    """
    from app.models.pamphlet import Pamphlet
    ...
    is_fork = db.query(PamphletFork).filter(
        ((PamphletFork.original_pamphlet_id == source.id) & (PamphletFork.new_pamphlet_id == target.id)) |
        ((PamphletFork.original_pamphlet_id == target.id) & (PamphletFork.new_pamphlet_id == source.id))
    ).first()
...
def get_pamphlet_merge_requests(
    ...
    merge_requests = db.query(MergeRequest).filter(
        ((MergeRequest.source_pamphlet_id == pamphlet_id) | (MergeRequest.target_pamphlet_id == pamphlet_id)) &
...
def update_merge_request(
    ...
    target_pamphlet = db.query(Pamphlet).filter(Pamphlet.id == merge_request.target_pamphlet_id).first()
    ...
    if update_data.status == MergeRequestStatus.APPROVED:
        ...
        source_version = db.query(PamphletVersion).filter(
            PamphletVersion.pamphlet_id == merge_request.source_pamphlet_id
        ).order_by(PamphletVersion.version_number.desc()).first()
